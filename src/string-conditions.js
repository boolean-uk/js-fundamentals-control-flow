...
let answerFour
if (
  STR_FIVE.charAt(0).toLocaleLowerCase() ===
  STR_FIVE.charAt(STR_FIVE.length - 1).toLocaleLowerCase()
)
  answerFour = true
...
let answerFive
if (
  STR_SIX.charAt(0).toLocaleLowerCase() ===
  STR_SIX.charAt(STR_SIX.length - 1).toLocaleLowerCase()
)
  answerFive = true
...
let answerSix
if (STR_SEVEN.length % 1 === 0 && STR_SEVEN.length !== 0) {
  answerSix = STR_SEVEN.at(STR_SEVEN.length / 2)
}
...
if (STR_EIGHT.length % 2 === 0) {
  answerSeven = STR_EIGHT.substring(STR_EIGHT / 2, STR_EIGHT / 2 + 1)
}
...
const winter = ['Desember', 'January', 'February']
