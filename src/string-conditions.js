...
let answerFour
if (STR_FIVE.charAt(0).toLowerCase() === STR_FIVE.charAt(STR_FIVE.length - 1)) {
  answerFour = true
}
...
let answerFive
if (STR_SIX.charAt(0).toLowerCase() !== STR_SIX.charAt(STR_SIX.length - 1)) {
  answerFive = false
} else {
  answerFive = true
}
...
if (STR_SEVEN.length % 2 !== 0) {
  const i = (STR_SEVEN.length - 1) / 2
  answerSix = STR_SEVEN.slice(i, i + 1)
}
...
if (STR_EIGHT.length % 2 === 0) {
  const i = (STR_EIGHT.length - 1) / 2
  answerSeven = STR_EIGHT.slice(i, i + 2)
}
