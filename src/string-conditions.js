...
if(STR_TWO.length>STR_FOUR.length){
  answerThree = true;
}
...
const strFiveLower = STR_FIVE.toLowerCase();
if(strFiveLower.charAt(0) === strFiveLower.charAt(strFiveLower.length - 1)){
  answerFour = true;
}
...
let STR_SIX = 'Joanna' // eslint-disable-line no-unused-vars

let answerFive = false;
const strSixLower = STR_SIX.toLowerCase();
if(strSixLower.charAt(0) === strSixLower.charAt(strSixLower.length - 1)){
  answerFive = true;
}

// 6. Use conditional statements to set answerSix to the middle character of STR_SEVEN
// if STR_SEVEN has an odd number of characters
let STR_SEVEN = 'Kayla' // eslint-disable-line no-unused-vars
...
if(STR_EIGHT.length % 2 === 0){
  const midStart = (STR_EIGHT.length / 2)- 1;
  answerSeven = STR_EIGHT.substring(midStart, midStart + 2);
}
