...
const strLow = STR_FIVE.toLocaleLowerCase()

let answerFour
if (strLow[0] === strLow[strLow.length - 1]) {
  answerFour = true
}
...
const STR_LOW = STR_SIX.toLowerCase()

let answerFive
if (STR_LOW[0] === STR_LOW[STR_LOW.length - 1]) {
  answerFive = true
} else {
  answerFive = false
}
...
// Check if STR_EIGHT has an even number of characters
if (STR_EIGHT.length % 2 === 0) {
  const middleIndex1 = STR_EIGHT.length / 2 - 1
  const middleIndex2 = STR_EIGHT.length / 2
  answerSeven = STR_EIGHT.slice(middleIndex1, middleIndex2 + 1)
}
