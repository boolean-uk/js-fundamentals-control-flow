...
let answerFour
if (STR_FIVE[0].toLowerCase(-1).toLowerCase()) {
  answerFour = true
}
...
let answerFive
const lastIndex = STR_SIX.length - 1
if (STR_SIX.toLowerCase()[0] === STR_SIX.toLowerCase()[lastIndex]) {
  answerFive = true
} else {
  answerFive = false
}
...
if (STR_EIGHT.length % 2 === 0) {
  const mid = STR_EIGHT.length / 2
  answerSeven = STR_EIGHT[mid - 1] + STR_EIGHT[mid]
}
