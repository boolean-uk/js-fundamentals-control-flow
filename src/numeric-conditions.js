...
let answerOne
answerOne = ARRAY_ONE.length === 0 ? (answerOne = true) : (answerOne = false)
console.log(answerOne)

// 2. Use conditional statements to set answerTwo to false if ARRAY_TWO is not empty
// or true if it is empty
let answerTwo
answerTwo = ARRAY_TWO.length === 0 ? (answerTwo = true) : (answerTwo = false)
console.log(answerTwo)

// 3. Use conditional statements to set answerThree to true if NUM_ONE is more than NUM_TWO
let answerThree
answerThree = NUM_ONE > NUM_TWO ? (answerThree = true) : (answerThree = false)
